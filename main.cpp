...
void intializeplayerplace(int r, int c)
{
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
...
void initializeenemyboard(int r, int c)
{
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        ...
        while (done != true)
        {
            ...
            {    //user has not entered valid coordinates for the orientation of the ship based on horizontal/vertical alignment or coordinates, or ship overlap
                cout << " you made an inaccurate assignment of the front of the ship."
                    ...
                    cout<<"\nIf you think there seems to be an error and you want to restart placing the ships enter 1 otherwise if you want to continue enter 0= ";
                    cin>>k;
                    if (k==1)
                    {
                        count = 0;
                        shipNumber = 0;
                        intializeplayerplace(row,col);
                    }
...
void level(int n, int m)
{
    ...
    if (n == 1) //EASY
    {
        ...
        if (m == 1)
        {
            ...
            system("cls");   //now that all the ships has been placed time to system cls
...
void credits()
{
    cout << "\nYou've successfully played the game.\n The game was made by Habiba Saleem and Kashif Ali from BSCS-A.";
}
